...
public class ExcelUtil {
     ...
     * @date 2014-7-8
     */
    public void assembleLocationSheet(List<SensorinfoVO> sensorinfoList,
    ...
    private void assembleTitleRow(Row titleRow,
                                  List<SensorinfoVO> sensorinfoList, String titleValue) {
    ...
    private void assembleLocationHeadRow(Row headRow,
                                         List<SensorinfoVO> sensorinfoList) {
        ...
        if (sensorinfoList != null && sensorinfoList.size() != 0) {
            for (int i = 1; i < sensorinfoList.size() + 1; i++) {
                SensorinfoVO sensorinfo = sensorinfoList.get(i - 1);
    ...
    private void assembleDataRow(Row dataRow,
                                 List<SensorinfoVO> sensorinfoList, RecentDataVO historyData) {
        ...
        for (int i = 1; i < sensorinfoList.size() + 1; i++) {
            SensorinfoVO sensorinfo = sensorinfoList.get(i - 1);
            LocationDataVO deviceData = locationDateMap.get(sensorinfo
                    .getSensorPhysicalid());
            ...
            cellTemp.setCellStyle(createNumericCellStyle());
        }
