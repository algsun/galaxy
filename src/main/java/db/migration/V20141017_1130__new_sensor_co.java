 ...
 * @date 2014-09-05
 */
public class V20141017_1130__new_sensor_co implements JdbcMigration {
    ...
    private static void addNewSensor(Connection connection) {
        if (!isDateExist(connection, QUERY_SENSOR_INFO + 2056)) {
            insertData(connection, INSERT_SENSOR_INFO + "  (2056, 'CO-HS', '一氧化碳-高灵敏度', 0, 'ppb', 0, 1, 0, 0, 25000, 3, 0)");
        }
    }

    private static void addNewFormulaSensor(Connection connection) {
        if (!isDateExist(connection, QUERY_FORMULA + 2056)) {
            insertData(connection, convertFormulaSql(2056));
        }
    }

    private static void addNewFormulaParam(Connection connection) {
        if (isNotExistFormulaParam(connection, 2056)) {
            insertData(connection, convertFormulaParamSql(2056));
        }
    }
    ...
    private static String convertFormulaSql(int sensorId) {
        return "INSERT INTO `m_formula_sensor`\n" +
                ...
                "             `y_range_type`,\n" +
                "             `sign_type`)\n" +
                "VALUES ( " + sensorId + ", 1, 0, 0, 0, 0, 25000, 3, 0)";
    }
    ...
    public static void main(String[] args) throws SQLException {
        Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/galaxy_bmy2", "root", "admin");
        addNewSensor(connection);
        addNewFormulaSensor(connection);
        addNewFormulaParam(connection);
    }
