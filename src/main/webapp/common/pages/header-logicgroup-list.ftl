...
<#if Session["currentSubLogicGroups"]??>
    <#list Session["currentSubLogicGroups"] as logicGroup>
        <li>
            <a href="${switch_logicgroup_url(logicGroup, subsystem)}">${logicGroup.logicGroupName}</a>
        </li>
    </#list>
</#if>
...
<#--用户归属站点 (多余的判断是过滤超级管理员)-->
    <li class="divider"></li>
    <li>
        <a href="${switch_logicgroup_url(Session["userLogicGroup"], subsystem)}">
        ${Session["userLogicGroup"].logicGroupName}<span class="muted">(归属站点)</span></a>
    </li>
</#if>
    <li class="divider"></li>
    <li><a href="#" onclick="javascript: App('common').header.chooseLogicGroup('${subsystem}'); return false;">其它站点...</a></li>
    <li><a href="#" onclick="javascript: SWITCH_MAP.logicGroupMap('${subsystem}'); return false;">站点地图</a></li>
</ul>
