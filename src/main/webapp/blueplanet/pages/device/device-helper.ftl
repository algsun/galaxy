...
<span class="muted <#if short>hide</#if>">${nodeId?substring(0, 8)}</span><strong>${nodeId?substring(8)}</strong>
</#macro>
...
<#-- 超时 -->
    <#if device.anomaly == -1>
        <@_deviceStateVew device, "label-important", locale.getStr("blueplanet.device.anomaly.overtime")/>
    <#-- 正常 -->
...
<#macro deviceWorkInterval workInterval>
    ...
    <#elseif (workInterval < 3600)>
    ${workInterval/60} ${locale.getStr("common.minute")}
    <#else>
    ${workInterval/3600}${locale.getStr("common.hour")}
    </#if>
</#macro>
