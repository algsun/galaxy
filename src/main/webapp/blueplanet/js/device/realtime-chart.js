...
$(function () {
    ...
    // 获取设备id
    var deviceId = $("#device-id").attr("data-device-id");
    ...
    $sensorinfoSelect.change(function () {
        ...
        initRealtimeChart(deviceId, sensorPhysicalId, cnName, units, function (newChart) {
            chart = newChart
        });
    }).change();
    ...
    $refreshIntervalSelect.change(function () {
        if (chart) {
            ...
            var sensorPhysicalId = $sensorinfoSelect.val();
            onChartLoad(deviceId, sensorPhysicalId, lastTime).apply(chart);
        }
    });


    // 初始化图表
    function initRealtimeChart(deviceId, sensorPhysicalId, sensorCnName, units, onComplete) {
        // 加载初始数据
        $.getJSON("device/" + deviceId + "/realtime-data.json", {count: DEFAULT_DATA_COUNT}, function (result) {
            var data = [];
    ...
    function optionFactory() {
        ...
        return {
            chart: {
                ...
                marginRight: 10,
                events: {
                    load: onChartLoad(option.deviceId, option.sensorPhysicalId, option.data[option.data.length - 1].x)
                }
            },
    ...
    function onChartLoad(deviceId, sensorPhysicalId, timeAfter) {
        ...
        return function () {
            ...
            fetchDataIntervalHook = setInterval(function () {
                // 请求最新的数据
                $.getJSON("device/" + deviceId + "/realtime-data.json", {startTime: lastTime}, function (result) {
                    if (result.length > 0) {
                        //记录最新的数据时间, 用于下一次请求
