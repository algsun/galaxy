...
//    var BluePlanet = App("blueplanet");
    ...
    $(document).on("click", ".new-ok", function () {
        ...
        var $zoneName = $block.find("#zoneName");
        if ($.trim($zoneName.val()).length < 1) {
            $zoneName.val("新建区域");
        }
        var zoneName = $zoneName.val();
        var position = $block.find("input[name='position']:checked").val();
        $.post("zone/add.json", {zoneName: zoneName, parentZoneId: currentZoneId, position: position}, function (result) {
            if (result.success) {
...
//                        BluePlanet.deviceTree.reload();
    ...
    $("#current-zone-add-child").click(function () {
        ...
        var $newZoneInput = $newZoneView.find("#zoneName");
    ...
    $(document).on("click", ".zone-move", function () {
        ...
        // 初始化 zTree, 加载顶级区域
        $.getJSON("zone/children.json", {zoneId: ""}, function (result) {
            var setting = {
                view: {
                    showLine: false
                },
...
//                BluePlanet.deviceTree.reload();
    ...
    $(document).on("click", ".zone-alter-submit>input", function () {
        ...
        var pPosition = $parent.find("input[name='position']:checked").val();
        $.post("zone/update.json", {image: pImage, zoneName: pZoneName, zoneId: pZoneId, position: pPosition}, function (result) {
            if (result.success) {
