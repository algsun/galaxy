            ...
            <div class="span12">
                ...
                <div id="destinations">
                    <#if (pathList?size>0)>
                        ...
                        <#if path.dataType==1>
                            <@destinationInput path.destinationName/>
                        </#if>
                    </#list>
                    <#else>
                        <@destinationInput/>
...
<div id="destinationLayout" class="hide">
    <div id="destination" class="control-group">
        <input name="destination" type="text" class="span10" placeholder="请输入目的地"/><span class="span2 pull-right"
...
$(function () {
    ...
    var map;
    var currentSiteLocation = new google.maps.LatLng($("#Lat").val(), $("#Lng").val());
    ...
    function loadPath() {
        ...
        // 是否存在预设线路
        if ($("#path").val() !== "") {
            destinations = JSON.parse($("#path").val());
        }
        $.each(destinations, function (i, destination) {
            location = new google.maps.LatLng(destination.latitude, destination.longitude);
            waypoints.push({location: location, stopover: (destination.dataType == 1 ? true : false)});
        });
...
// 添加目的地
    $("#addDestination").click(function () {
        $("#destinations").append($("#destinationLayout").children().clone());
...
// 保存预设线路
    $("#btnSave").click(function () {
        ...
        $.post("routePlanning/exhibition/" + exhibitionId + "/save", {path: destinations, point: gpsPoints}, function (result) {
            if (result.success) {
...
// 更新 wayPoints
    $("#btnQuery").click(function () {
        ...
        var waypoints = new Array($destinations.length);
        $.each($destinations, function (i, destination) {
            ...
            if ($.trim(destination.value) === "") {
                ...
                geocoder.geocode({ 'address': $destination.val()}, function (results, status) {
                    if (status == google.maps.GeocoderStatus.OK) {
                        waypoints[i] = ({location: results[0].geometry.location});
                    }

                    if (waypoints.length === $destinations.length) {
                        calcRoute(waypoints);
                    }
                });
            }
...
<#macro destinationInput destinationName=''>
<div id="destination" class="control-group">
    <input name="destination" type="text" class="span10" placeholder="请输入目的地" value='${destinationName}'/><span
