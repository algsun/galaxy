...
<div id="gcontent" class="container m-t-50">
    <form class="form-horizontal" id="repair_record_form" action="repairRecords" method="post">
        <input type="hidden" name="repairRecord.id" value="${repairRecord.id?c}"/>
        ...
        <fieldset>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    ...
                    <#list repairReasons as repairReason>
                        <option value="${repairReason.id}" <#if repairReason.id == repairRecord.repairReason.id>
                                selected="selected"</#if> >${repairReason.reason}</option>
                    </#list>
                    </select>
                </div>
            </div>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <input id="returnDate" type="text" value="${repairRecord.returnDate?string("yyyy-MM-dd")}"
                           onclick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'extractDate\')}',maxDate:'2100-12-31'})"
                           name="repairRecord.returnDate"/>
                </div>
            </div>
