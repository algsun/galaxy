...
<table border="1px" style="border:2px #000 solid;">
    ...
    <tr style="height: 400px">
        <td colspan="20">${repairRecord.situation.summarize!}</td>
    </tr>
    <tr>
        <td colspan="4">技术变更</td>
        <td colspan="16">${repairRecord.situation.techChange!}</td>
    </tr>
    <tr>
    ...
    <#if repairLogs??>
        <#list repairLogs as repairLog>
            <tr>
                <td colspan="3">保护修复人</td>
                <td colspan="7">${repairLog.repairPerson.userName!}</td>
                <td colspan="3">日期</td>
                <td colspan="7">${repairLog.stamp?string("yyyy-MM-dd")}</td>
            </tr>
            <tr>
                <td colspan="6">日志</td>
                <td colspan="14">${repairLog.log!}</td>
            </tr>
        </#list>
    </#if>
    <#if repairPhotos??>
        <#list repairPhotos as repairPhoto>
            <tr>
                <td colspan="20">
                    <img src="${picturesBasePath!}/${siteId!}/${repairRecord.relic.totalCode!}/${repairPhoto.path}" style="max-width: 100px;max-height: 100px; text-align: center"/><br/>
    ...
    <table border="1px" style="border:2px #000 solid;">
        ...
        <tr style="height: 300px">
            <td colspan="20" class="none-border">
                <textarea name="repairAssessment.selfAssessment" id="selfAssessment" cols="30" rows="12"
                          style="width: 95%">${repairAssessment.selfAssessment?string("yyyy-MM-dd")}</textarea>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td colspan="4" class="none-border">
                <input name="repairAssessment.stamp" id="assessmentStamp" type="text" value="${repairAssessment.stamp!}"
